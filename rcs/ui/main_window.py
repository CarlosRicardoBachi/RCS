...
class MainWindow(QMainWindow):
    ...
    # HOTFIX Rustic-compatible GMPR import
    def _import_gmpr_rustic(self, gmpr_path):
        import json, tempfile, os
        from rcs.core.gmpr_io import load_gmpr_project

        data = load_gmpr_project(gmpr_path)

        # 1) Prepare SVG (embedded preferred)
        svg_path = data.get("svg_path")
        if data.get("svg_embedded"):
            fd, tmp = tempfile.mkstemp(suffix=".svg")
            os.close(fd)
            with open(tmp, "w", encoding="utf-8") as f:
                f.write(data["svg_embedded"])
            svg_path = tmp

        # 2) Load SVG without auto-fit
        self._load_svg_path(svg_path)

        # 3) Do NOT rescale / fit: GMPR defines world
        # 4) Create raster objects exactly as stored
        for obj in data.get("objects", []):
            if obj.get("type") == "raster":
                item = self.canvas_view.create_raster(obj["path"])
                try:
                    item.setTransform(obj.get("transform"))
                except Exception:
                    pass

        self.rebuild_tree()
        self.mark_dirty("import_gmpr_rustic")
